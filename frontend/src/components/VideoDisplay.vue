...
<template>
    <div class="mx-auto space-y-8 bg-white rounded-lg shadow">
        ...
        <!-- Video Player -->
        <div class="p-6">
            <video controls class="w-full h-auto rounded-lg" :src="submissionVideo.video_url" type="video/mp4"
                @error="videoError = true" v-show="!videoError">
            </video>
            <div v-if="videoError" class="mt-4 text-red-600">
                Video cannot be played.
                <span>
                    <a :href="submissionVideo.video_url" target="_blank" class="underline text-blue-600">
